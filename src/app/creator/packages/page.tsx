      ...
      <div className="mx-auto max-w-4xl space-y-6">
        ...
        <div className="flex items-center justify-between gap-4">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              ...
              <p className="text-xs text-surface-500">
                Set rates and offerings for Human & AI UGC video campaigns
              </p>
            </div>
          </div>
          ...
          <button
            ...
            onClick={handleSave}
            disabled={saving}
            className="hidden sm:inline-flex items-center gap-2 rounded-xl bg-brand-600 hover:bg-brand-700 active:scale-95 text-white px-5 py-2.5 text-xs font-bold transition-all shadow-md shadow-brand-100/50 disabled:opacity-50 min-h-11"
          >
        ...
        {error && (
          <div className="rounded-2xl bg-red-50 p-4 text-xs text-red-600 font-medium border border-red-200">
            {error}
          </div>
        )}

        {success && (
          <div className="rounded-2xl bg-emerald-50 p-4 text-xs text-emerald-700 font-bold border border-emerald-200 flex items-center gap-2 shadow-2xs">
        ...
        <div className="rounded-2xl border border-surface-200 bg-white p-4 sm:p-6 shadow-2xs space-y-3">
          <div className="flex items-center gap-2">
            ...
            <label className="text-xs font-bold text-surface-900 uppercase tracking-wider block">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
            ...
            >
              ...
              <div>
                <p className="text-xs font-bold">Human UGC</p>
          ...
          <div className="flex gap-2 overflow-x-auto pb-1 scrollbar-hide -mx-1 px-1">
            {UGC_PACKAGE_TEMPLATES.map((t) => (
              <button
                ...
                className="px-4 py-2.5 rounded-full text-xs font-semibold border border-surface-200 hover:border-brand-300 hover:bg-brand-50 text-surface-700 disabled:opacity-40 disabled:cursor-not-allowed transition-all shrink-0 min-h-11"
              >
                + {t.label}
              </button>
            ))}
        ...
        <div className="rounded-2xl border border-surface-200 bg-white p-4 sm:p-6 shadow-2xs space-y-4">
          <div className="flex items-center justify-between">
            ...
            <button
              type="button"
              onClick={addPackage}
              className="inline-flex items-center gap-1.5 text-xs font-bold text-brand-600 hover:text-brand-700 transition-colors min-h-10"
            >
          ...
          {packages.length === 0 ? (
            <div className="py-12 text-center border-2 border-dashed border-surface-200 rounded-2xl px-4">
              <Clapperboard className="h-8 w-8 text-surface-300 mx-auto mb-2" />
              ...
              <p className="text-xs text-surface-500 mt-1 max-w-xs mx-auto">
                Use the Quick-Add Preset Templates above or add a custom video
                package to showcase your rates to brands.
              </p>
            </div>
          ) : (
            <div className="space-y-3">
              {packages.map((pkg, i) => (
                <div
                  key={i}
                  className="flex flex-col sm:flex-row gap-3 items-stretch sm:items-center rounded-2xl border border-surface-200 bg-surface-50/70 p-3.5 sm:p-4 space-y-2 sm:space-y-0"
                >
                  {/* Package Title */}
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <button
                      ...
                      className="text-surface-400 hover:text-red-600 transition-colors p-2.5 rounded-xl border border-surface-200 sm:border-0 bg-white sm:bg-transparent min-h-11 min-w-11 flex items-center justify-center shrink-0 mt-4 sm:mt-0"
