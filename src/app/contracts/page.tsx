    ...
    async function load() {
      ...
      if (prof.role === "brand") {
        const { data: camps } = await supabase
          .from("campaigns").select("*, milestones:milestones(*)").eq("brand_id", user.id)
            ...
            <Button
              ...
              onClick={() => {
                const data = contracts.map((c: any) => {
                  ...
                  const funded = milestones.filter((m: Milestone) => m.state !== "UNFUNDED").length
                  return {
                    Campaign: c.title,
              ...
              <Table>
                ...
                <TableBody>
                  {contracts.map((contract: any) => {
                    ...
                    const fundedCount = milestones.filter((m: Milestone) => m.state !== "UNFUNDED").length
                    return (
                      <TableRow key={contract.id}>
                        <TableCell className="font-medium">{contract.title}</TableCell>
                        <TableCell>{formatCurrency(Number(contract.budget))}</TableCell>
                        <TableCell>{fundedCount}/{milestones.length} funded</TableCell>
                        <TableCell>
