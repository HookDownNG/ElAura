...
import { ArrowLeft, DollarSign, Calendar, Send, CheckCircle, AlertTriangle, ExternalLink } from "lucide-react"
...
export default function CampaignDetailPage() {
  ...
  async function handleApply() {
    ...
    if (!error) {
      setApplication({ id: "", campaign_id: campaignId, creator_id: user.id, message, status: "pending", created_at: new Date().toISOString() })
    }
    setSubmitting(false)
  }
  ...
  async function handleAcceptApplication(appId: string) {
    ...
    if (!error) {
      setApplication(prev => prev?.id === appId ? { ...prev!, status: "accepted" } : prev)
    }
  }
  ...
  const isBrandOwner = profile?.role === "brand" && profile.id === campaign.brand_id
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <Card>
              ...
              <CardContent className="space-y-3">
                {milestones.map((ms) => (
                  <div key={ms.id} className="rounded-lg border border-border p-4">
                ...
                <CardContent className="space-y-3">
                  {application ? (
                    <div className="space-y-2">
                      ...
                      >
                        {application.status === "accepted" ? "Accepted" : application.status === "rejected" ? "Rejected" : "Application Submitted"}
                      </Badge>
                    </div>
                  ) : (
                    <>
