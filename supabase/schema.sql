...
-- Creator profiles: only the creator can view/update their own
CREATE POLICY "Creator profiles viewable by owner" ON public.creator_profiles
  FOR SELECT USING (auth.uid() = id);

CREATE POLICY "Creator profiles updatable by owner" ON public.creator_profiles
  FOR UPDATE USING (auth.uid() = id);

CREATE POLICY "Creator profiles insertable by owner" ON public.creator_profiles
  FOR INSERT WITH CHECK (auth.uid() = id);

-- Brand profiles: viewable by everyone, updatable by owner
CREATE POLICY "Brand profiles viewable by everyone" ON public.brand_profiles
  FOR SELECT USING (true);

CREATE POLICY "Brand profiles updatable by owner" ON public.brand_profiles
  FOR UPDATE USING (auth.uid() = id);

CREATE POLICY "Brand profiles insertable by owner" ON public.brand_profiles
  FOR INSERT WITH CHECK (auth.uid() = id);
